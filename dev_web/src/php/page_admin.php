        ...
        <div class="admin-main">
            <div class="recherche">
                ...
                <form method="post" id='post-admin-up'>
                    <input class='btn-simple-bleu' id='usexr' type='submit' name="users" value="Utilisateurs">
            ...
            <form method="post" id=''>
                <select name="choix_tri" id="choix_tri">
                    ...
                    <option value="order_by_asc">Ordre alphabétique</option>
                    <option value="order_by_desc">Ordre anti alphabétique</option>
                </select>
            ...
            <?php
                ...
                if (isset($_POST['submit_tri']) && isset($_POST['choix_tri']) && isset($_COOKIE["table"])){
                    $choix = $_POST['choix_tri'];
                    $table = $_COOKIE["table"];

                    if ($choix == "order_by_asc"){
                        $choix = 'ORDER BY login ASC';
                    }
                    if ($choix == "order_by_desc"){
                        $choix = 'ORDER BY login DESC';
                    }
                    affichage($table,$choix);
                }
            ?>
        </div>
...
<?php
    ...
    function affichage($table,$tri)
    {
        ...
        if ($table == "users") {
            ...
            $requete1 = mysqli_query($connexion,"SELECT id_user, login, type_user, nb_visites from $table '".$tri."' ");
        }
        //Si la table est activiteconnexion.
        else if ($table == "activiteconnexion") {
            //On récupère toutes les information dont ont à besoin.
            $requete1 = mysqli_query($connexion,"SELECT id_connexion, mdp_tente, login, date_horaire_tent, adr_ip from $table '".$tri."'");
        }
        //Si la table est activitemodule.
        else if ($table == "activitemodule") {
            //On récupère toutes les information dont ont à besoin.
            $requete1 = mysqli_query($connexion,"SELECT id_activite, id_module, login from $table '".$tri."'");
        }
        //Si la table est le dernier bouton.
        else {
            //On récupère toutes les information dont ont à besoin.
            $requete1 = mysqli_query($connexion,"SELECT * from $table '".$choix."'");
        }
